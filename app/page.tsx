          ...
          {display.map((p, idx) => (
            <HoverRow key={idx} selected={selectedIdx === projects.indexOf(p)} onClick={() => setSelectedIdx(projects.indexOf(p))}
              ...
              {visibleItems.has("Clients") && (
                <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 600, color: t.fg, background: t.muted, padding: "6px 12px", borderRadius: 8, cursor: "pointer", border: `1px solid ${t.border}` }}>{clients[p.clientId]?.name}</span>
              )}
              <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 600, color: t.fg, background: t.muted, padding: "6px 12px", borderRadius: 8, cursor: "pointer", border: `1px solid ${t.border}` }}>{p.office}</span>
              {visibleItems.has("Business units") && (
                <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 600, color: t.fg, background: t.muted, padding: "6px 12px", borderRadius: 8, cursor: "pointer", border: `1px solid ${t.border}` }}>{p.unit || "—"}</span>
              )}
              <span style={{ display: "flex", alignItems: "center" }}>
                <div style={{ width: 24, height: 24, borderRadius: "50%", background: t.muted, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 600, color: t.fg }}>
                  {people[p.ownerId]?.name.charAt(0) || "?"}
